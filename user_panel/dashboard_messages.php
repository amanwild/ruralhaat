        ...
        <div class="row">
          <div class="col-lg-12 col-md-12">
            <div class="utf_dashboard_list_box margin-top-0">
              ...
              <?php $select_messaege_query = 'SELECT *,max(message.message_timestamp)as timestamp FROM `message` left join users_entries on message.message_sender_user_id = users_entries.user_id left join enquery on message.message_sender_enquery_id = enquery.enquery_id WHERE message.message_receiver_user_id = '.$_SESSION['user_id'].' GROUP by message.message_sender_enquery_id, message.message_sender_user_id';
              try {
                echo $select_messaege_query;
                $select_messaege_result = 0;
                if ($connect) {
                  $select_messaege_result = mysqli_query($connect, $select_messaege_query);
                  if ($select_messaege_result) {
                    $select_messaege_num = mysqli_num_rows($select_messaege_result);
                  }
                }
              } catch (Exception $e) {
                $mess = $e->getMessage();
              ...
              <div class="utf_user_messages_block">
                <ul class="paginationTable">
                  ...
                  if ($select_messaege_num > 0) {
                    while ($row_for_messaege = mysqli_fetch_assoc($select_messaege_result)) {
                      // $interaction_listing_title = $row_for_messaege['listing_title'];
                      if ($row_for_messaege['user_type'] != '') {
                          ...
                          <a href="dashboard_messages_details.php">
                            ...
                            <div class="utf_message_headline_item">
                              <div class="utf_message_headline_text">
                                <h5>User <?= $row_for_messaege['user_first_name'] ?> <?= $row_for_messaege['user_last_name'] ?>
                                  (<?php if ($row_for_messaege['user_type'] == 'user') {
                                      echo 'seller';
                                    } else {
                                      echo $row_for_messaege['user_last_name'];
                                    } ?>) <i>New</i></h5>
                                <span><i class="fa fa-clock-o"></i> Jan 05, 2022 - 8:52 am</span>
                              </div>
                              <span><b><?= $row_for_messaege['message_subject'] ?> :</b></span>
                              <span><?= $row_for_messaege['message_massage'] ?></span>
                            </div>
                          </a>
                        </li>
                      <?php
                      ...
                      ?>
                        <li class="unread tableItem">
                          <a href="dashboard_messages_details.php">
                            ...
                            <div class="utf_message_headline_item">
                              <div class="utf_message_headline_text">
                                <h5><?php echo $row_for_messaege['enquery_first_name']; ?> <?= $row_for_messaege['enquery_last_name'] ?>(stranger) <i>New</i></h5>
                                <span><i class="fa fa-clock-o"></i> Jan 05, 2022 - 8:52 am</span>
                              </div>
                              <span><b><?= $row_for_messaege['message_subject'] ?> :</b></span>
                              <span><?= $row_for_messaege['message_massage'] ?></span>
