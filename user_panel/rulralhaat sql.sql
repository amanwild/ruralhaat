...
SELECT *,
    count(message_id)
FROM `message`
    left join users_entries on message.message_sender_user_id = users_entries.user_id
    left join enquery on message.message_sender_enquery_id = enquery.enquery_id
WHERE `message_receiver_user_id` = 166
GROUP by message.message_sender_enquery_id,
    message.message_sender_user_id
order by message_timestamp


SELECT message.message_sender_enquery_id,
    message.message_sender_user_id,
    count(message_id)
FROM `message`
    left join users_entries on message.message_sender_user_id = users_entries.user_id
...
FROM `message`
    ...
    left join enquery on message.message_sender_enquery_id = enquery.enquery_id
WHERE message.message_receiver_user_id = 166
ORDER BY `message`.`message_timestamp` DESC
...
FROM `message`
    left join users_entries on message.message_sender_user_id = users_entries.user_id
    left join enquery on message.message_sender_enquery_id = enquery.enquery_id
WHERE message.message_receiver_user_id = 166 and mes
ORDER BY `message`.`message_timestamp` DESC
