...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    ...
    if ($num > 0) {
        ...
        $insert_query = "UPDATE `users_entries` SET `user_password` = '$hash',`user_type` = 'user',`user_request_for_seller` = '0', `is_verified_admin` = '1', `user_admin_verified_timestamp`=current_timestamp() WHERE `users_entries`.`user_id` = $id";
        try {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['array_of_checked_id'])) {
    ...
    foreach ($array_of_checked_id as $id) {
        ...
        $update_query = "UPDATE `users_entries` SET `user_password` = '$hash',`user_type` = 'user',`user_request_for_seller` = '0', `is_verified_admin` = '1', `user_admin_verified_timestamp`=current_timestamp() WHERE `users_entries`.`user_id` = $id ; ";
