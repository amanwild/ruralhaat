...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"])) {
    ...
    if ($num > 0) {
        ...
        try {
            $update_result = false;
            $update_query = "UPDATE `users_entries` SET `is_verified_admin` = '1' ,`user_type` = 'buyer',`user_request_for_seller` = '0', `user_admin_verified_timestamp`=current_timestamp() WHERE `users_entries`.`user_id` = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['array_of_checked_id'])) {
    ...
    foreach ($array_of_checked_id as $id) {
        ...
        $update_query = "UPDATE `users_entries` SET `user_password` = '',`user_type` = 'buyer',`user_request_for_seller` = '0', `is_verified_admin` = '2' , `user_admin_verified_timestamp`=current_timestamp() WHERE `users_entries`.`user_id` = $id ; ";
        try {
            echo $update_query;
            $update_result = mysqli_query($connect, $update_query);
        } catch (Exception $e) {
