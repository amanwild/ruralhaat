...
<body data-rsssl=1 class="page-template page-template-template-login page-template-template-login-php page page-id-49 theme-classiera woocommerce-no-js single-author elementor-default elementor-kit-1989">
  ...
  <div id="showLocatonContent" class="d-flex text-center mt-3 ">
    <h1 class=" my-auto mr-auto d-inline"> User Conversion</h1>
  </div>
      ...
      <table data-role="table" class="table ui-responsive datatable  " id="myTable">
        <div id="showEditorOption" style="float:left;display:none !important;" class="d-flex text-center">
          <div class="form-group">
            <button type="" name="submit" onclick="approve_ac_with_checkbox_for_conversion()" style='background-color:green;color:white;padding:5px;font-weight:700;font-size:15px;' class="btn  sharp btn-md btn-style-one" value="Send">Approve</button>
            <button type="" style='background-color:red;color:white;padding:5px;font-weight:700;font-size:15px;' name="submit" class="btn  sharp btn-md btn-style-one" onclick="reject_ac_with_checkbox_for_conversion()" value="Send">Reject</button>
          </div>
        </div>
        ...
        <tbody>
          ...
          $select_user_query = "SELECT * FROM `users_entries` WHERE (user_type like 'buyer' or user_type like 'user') and is_verified_email = 1  ORDER BY `users_entries`.`user_last_timestamp` DESC";
          // echo$fname,$lname;
          try {
              ...
              <tr id="row_<?= $row['user_id'] ?>" class="row_by_id" style="">
                ...
                <td id="status_<?= $row["user_id"] ?>">
                  ...
                  if ($row["user_type"] =="user") {
                    echo "<span style='background-color:green;color:white;padding:5px;font-weight:700;border-radius:5px;'>Seller</span>";
                  } else if ($row["user_type"] =="buyer" && $row["user_request_for_seller"] =="0") {
                    echo "<span style='background-color:red;color:white;padding:5px;font-weight:700;border-radius:5px;'>Buyer</span>";
                  }  else if ($row["user_type"] =="buyer" && $row["user_request_for_seller"] =="1" ) {
                    echo "<span style='background-color:#f1c40f;color:white;padding:5px;font-weight:700;border-radius:5px;'>Pending</span>";
                  }
                  ?></td>
                <td class="text-center">
                  <span type="button" style="cursor: pointer;width:50px;margin-right:0px;font-size:25px !important;" id="approve_<?= $row["loc_id"] ?>" class="d-inline edit table_edit_btn" data-bs-toggle="modal" data-bs-target="#editModal" onclick="approve_ac_for_conversion(<?= $row['user_id'] ?>)"> ✅</span>
                  <span style="cursor: pointer;width:50px;margin-right:0px;font-size:25px !important;" id="d<?= $row["user_id"] ?>" type="button" id="d" class="d-inline delete table_delete_btn " onclick="reject_ac_for_conversion(<?= $row['user_id'] ?>)"> ❌</span>
                </td>
              </tr>
          <?php
            }
