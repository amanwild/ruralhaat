...
FROM
  ...
  left join users_entries on users_entries.user_id = listing.listing_owner_id
  ...
  left join cities on listing.listing_city_id = cities.city_id where listing.listing_id = $enquery_listing_id"; //NOTE: here (`) is not equal to (')
      ...
      while ($row = mysqli_fetch_assoc($select_target_listing_result)) {
        ...
        $interaction_on_listing_listing_email = $row['user_email'];
        $interaction_on_listing_listing_phone = $row['user_phone'];
        $interaction_on_listing_listing_contact_name = $row['user_first_name']." ". $row['user_last_name'];
