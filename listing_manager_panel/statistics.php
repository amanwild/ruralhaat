...
<body data-rsssl=1 class="page-template page-template-template-login page-template-template-login-php page page-id-49 theme-classiera woocommerce-no-js single-author elementor-default elementor-kit-1989">
  ...
  <div id="showLocatonContent" class="d-flex text-center mt-3 ">
    <h1 class=" my-auto mr-auto d-inline"> All Statistics</h1>
  </div>
      ...
      <table data-role="table" class="table ui-responsive datatable  " id="myTable">
        <thead>
          <tr>
            ...
            <?php $select_category_query = "SELECT * FROM `category` ORDER BY `category`.`category_name` ASC";
            // echo$fname,$lname;
            try {
        ...
        <tbody>
          ...
          $select_query = "SELECT * FROM `users_entries` WHERE user_type LIKE 'user' AND is_verified_email = 1";
          // echo$fname,$lname;
          try {
          ...
          if ($num > 0) {
            ...
            while ($row = mysqli_fetch_assoc($select_result)) {
              ...
              $select_listing_query = "SELECT *, COUNT(listing_owner_id) as all_category_count FROM `listing` WHERE listing_owner_id = $user_id";
              // echo$fname,$lname;
              try {
              ...
              if ($num > 0) {
                while ($row = mysqli_fetch_assoc($select_listing_result)) {
                  $all_category_count = $row["all_category_count"];
                }
              }
          ...
          ?>
              <tr>
                ...
                if ($num_for_category > 0) {
                  ...
                  while ($row = mysqli_fetch_assoc($select_category_result)) {
                    $select_count_by_category_query = "SELECT count(category.category_id) as category_" . $row["category_id"] . "_count FROM users_entries left join listing on users_entries.user_id = listing.listing_owner_id LEFT join category on listing.listing_category_id = category.category_id WHERE users_entries.user_id = $user_id and listing.listing_category_id= " . $row["category_id"] . " ORDER BY `category`.`category_name` ASC";
                    // echo$fname,$lname;
                    try {
                ...
                ?>
                        <td><?= $row_for_category_count["category_" . $row["category_id"] . "_count"] ?></td>
                ...
                <td><img src="../wp-content/uploads/data/<?= $user_image ?>" class="rounded-circle img-fluid" style="object-fit:cover;max-width: 32px;height: auto;" class="mr-3 p-1" alt="." /></td>
