  ...
  <div id="main_wrapper">
    ...
    <div id="dashboard">
      ...
      <div class="utf_dashboard_content">
        ...
        if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST["request_for_seller_ac"])) {
          // echo "hello";

          try {
            $request_for_seller_query = "UPDATE `users_entries` SET `user_request_for_seller` = '1' WHERE `users_entries`.`user_id` = " . $_SESSION['user_id'];
            $request_for_seller_result = mysqli_query($connect, $request_for_seller_query);

            // echo $request_for_seller_query;
            if ($request_for_seller_result) {
              echo '<div class="row"> <div class="col-md-12"> <div class="notification success closeable margin-bottom-30"> <p> Request is Sent Successfully </p> <a class="close" href="#"></a> </div> </div> </div>'; // echo ("<br> email shooting successfull <br>");

              // echo ("<br> email shooting successfull <br>");
              $_SESSION['user_request_for_seller'] = 1;
            } else {
        ...
        if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST["addList"]) && isset($_POST['listing_title'])) {
            ...
            <div class="col-lg-12">
              ...
              <div id="utf_add_listing_part">
                <form enctype="multipart/form-data"  method="POST" action="<?= $_SERVER["REQUEST_URI"]; ?>">
                    ...
                    <div class="row with-forms">
                      ...
                      if ($num > 0) {
                        while ($row = mysqli_fetch_assoc($select_result)) {
                          $user_request_for_seller = $row['user_request_for_seller'];
                        }
                      }
                      if ($user_request_for_seller) {
                      ?>
                        <input type="text" placeholder="You have Applied for Seller Account. Your Request Status is Pending." disabled />
                      <?php
                      } else {
                      ?>
                        <input type="text" placeholder="Apply Now and become Seller within 24hr" disabled />
                        <input type="hidden" name="request_for_seller_ac" id="request_for_seller_ac" value="true" />
                      <?php
                      } 
                      ?>
                    </div>
                  ...
                  if ($user_request_for_seller) {
                  ?>
                    <button type="submit" class="button preview" id="submit" disabled>Already Applied</button>
                  <?php
                  } else {
                  ?>
                    <button type="submit" class="button preview" id="submit">Apply</button>
